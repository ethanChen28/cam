...
vector<STrack> BYTETracker::update(const vector<Object> &objects) {
  ...
  STrack::tracking_ids.clear();
  for (int i = 0; i < this->tracked_stracks.size(); i++) {
    if (this->tracked_stracks[i].is_activated) {
      STrack::tracking_ids.push_back(this->tracked_stracks[i].track_id);
    }
  }
  for (int i = 0; i < this->lost_stracks.size(); i++) {
    STrack::tracking_ids.push_back(this->lost_stracks[i].track_id);
  }
  ...
  for (int i = 0; i < removed_stracks.size(); i++) {
    output_stracks.push_back(removed_stracks[i]);
  }
  if (this->removed_stracks.size() > 256) {
