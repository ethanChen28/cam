...
typedef struct _webrtc_streamer_session_info{
	...
	int connect_time;        
	int session_time;  
        int video_send_bitrate;  //视频的实际发送码率  KB/S
        int audio_send_bitrate;  //音频的实际发送码率  KB/S
	int send_packets;      //发送包数
        int resend_packets;      //重传包数
        int current_resend_packets;  //当前重传包数
        int video_recv_bitrate;  //视频的实际接收码率  KB/S
        int audio_recv_bitrate;  //音频的实际接收码率  KB/S
        int transmission_mode;   //网络传输方式    0 未连接 ,1 p2p ,2 relay
        int audio_packet_loss;   //音频总丢包数
        int video_packet_loss;   //视频总丢包数
	int audio_current_packet_lost;  //音频当前丢包数
	int video_current_packet_lost;  //视频当前丢包数
...
typedef void(*webrtc_streamer_datachannel_message_callback)(char *sessionId,size_t sessionId_len,webrtc_data_message_type_t type,char *Msg,size_t Msg_len,void *user);
typedef void(*webrtc_streamer_datachnanle_open_callback)(char *sessionId,size_t sessionId_len,void *user);
...
*/
int webrtc_streamer_datachannel_send_message(char *sessionId,webrtc_data_message_type_t type,char *data,size_t len);
/*
获取会话信息函数
*/
...
int webrtc_streamer_call(char * to,size_t to_len);
